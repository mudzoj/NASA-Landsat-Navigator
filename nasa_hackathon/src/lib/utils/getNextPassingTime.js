...
export async function getNextPassingTime(path, row) {
  const pattern = /(\d{2,3})\s+(\d{2})\s+(\d{3}-\d{2}:\d{2}:\d{2})\s+(\w{3})/g;
  let day;
  let isFound = false;
  for (let i = 0; i < 16 && !isFound; i++) {
    for (let pathData of pathSchedule[i]) {
      if (pathData === path) day = i;
      isFound = true;
      break;
    }
  }
  ...
  const scheduleText = await fs.readFile(
    `../data/landsat-9-schedule/day-${day}.txt`,
    "utf-8"
  );

  let match;
  while ((match = pattern.exec(scheduleText)) !== null) {
    const [_, pathData, rowData, imageStart, station] = match;
    if (pathData === path && rowData === row) {
      return imageStart;
    }
  }
}
